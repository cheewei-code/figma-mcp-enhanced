...
function resolveFileKey(file_key) {
    ...
    if (!key)
        throw new Error("No file_key provided and FIGMA_FILE_KEY environment variable is not set.");
    return key;
}
// Resolve team ID from argument or FIGMA_TEAM_ID env var
function resolveTeamId(team_id) {
    const id = team_id ?? process.env.FIGMA_TEAM_ID;
    if (!id)
        throw new Error("No team_id provided and FIGMA_TEAM_ID environment variable is not set.");
    return id;
}
...
server.tool("get_team_components", "List all published components in a Figma team library. Requires FIGMA_API_TOKEN.", {
...
server.tool("get_project_structure", "Get all projects and their files for a Figma team. Requires FIGMA_API_TOKEN.", {
    team_id: z.string().optional().describe("Figma team ID. Defaults to FIGMA_TEAM_ID env variable."),
}, async ({ team_id }) => {
...
// Tool: list webhooks
server.tool("list_webhooks", "List all webhooks for a Figma team. Requires FIGMA_API_TOKEN.", {
    team_id: z.string().optional().describe("Figma team ID. Defaults to FIGMA_TEAM_ID env variable."),
}, async ({ team_id }) => {
...
// Tool: create webhook
server.tool("create_webhook", `Create a webhook for a Figma team. Requires FIGMA_API_TOKEN.
